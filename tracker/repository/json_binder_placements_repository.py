"""Lecture / écriture de `data/binder-placements.json`."""
...
class JsonBinderPlacementsRepository:
    def __init__(self, file_path: Path) -> None:
        self._path = file_path

    def load(self) -> BinderPlacementsPayload:
        if not self._path.is_file():
            return BinderPlacementsPayload()
        try:
        ...
        if not isinstance(raw, dict):
            return BinderPlacementsPayload()
        try:
            return BinderPlacementsPayload.model_validate(raw)
